...
function sendBaleMessage($chatId, $message) {
    ...
    if ($result === false) {
        writeLog("Failed to send message to Bale.ai");
        return false;
    }
    return true;
}

function sendBalePhoto($chatId, $photoUrl) {
    $url = "https://tapi.bale.ai/bot" . BOT_TOKEN . "/sendPhoto";
    $data = [
        'chat_id' => $chatId,
        'photo' => $photoUrl
    ];
    ...
    if ($result === false) {
        writeLog("Failed to send photo to Bale.ai");
        return false;
    }
    return true;
}
